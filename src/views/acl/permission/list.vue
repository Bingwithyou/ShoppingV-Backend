        ...
        <el-table border style="margin-bottom: 20px;" :data="menuPermissionList" :expand-row-keys="expandKeys"
            ...
            <el-table-column label="操作">
                <template slot-scope="{row}">
                    <el-button type="primary" icon="el-icon-plus" size="mini" @click="toAddPermission(row)"
                        :title="getAddTitle(row.level)"></el-button>

                    <el-button type="primary" icon="el-icon-edit" size="mini" @click="toUpdatePermission(row)"
                        :title="row.level === 4 ? '修改功能' : '修改菜单'"></el-button>

                    <el-button type="danger" icon="el-icon-delete" size="mini" @click="removePermission(row)"
                        title="删除"></el-button>
                </template>
            </el-table-column>
        </el-table>
...
<script>
import permissionRequest from '@/api/acl/permission'
    ...
    methods: {
        ...
        */
        async fetchPermissionList() {
            const result = await permissionRequest.getPermissionList()
        ...
        */
        removePermission(permission) {
            this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
                ...
                const result = await permissionRequest.removePermission(permission.id)
        ...
        addOrUpdatePermission() {
            this.$refs.permission.validate(async valid => {
                if (valid) {
                    ...
                    const result = await permissionRequest[perm.id ? 'updatePermission' : 'addPermission'](perm)
