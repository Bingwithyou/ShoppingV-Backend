<template>
    <div style="margin-top:20px">
        ...
        <el-table border stripe v-loading="listLoading" :data="roles"
            ...
            <el-table-column label="操作" >
                <template slot-scope="{row}">
                    ...
                    <el-button size="mini" type="danger" icon="el-icon-delete" title="删除角色"
                        @click="removeRole(row)"></el-button>
                </template>
            </el-table-column>
        </el-table>
...
<script>
import roleRequest from '@/api/acl/role'
export default {
    ...
    methods: {
        ...
        */
        updateRole(role) {
            roleRequest.updateById({ id: role.id, roleName: role.roleName })
                .then(result => {
        ...
        */
        addRole() {
            ...
            this.$prompt('请输入新名称', '添加角色', {
                ...
                roleRequest.save({ roleName: value }).then(result => {
        ...
        */
        getRoles(page = 1) {
            ...
            roleRequest.getPageList(page, limit, searchObj).then(
                result => {
        ...
        */
        removeRole({ id, roleName }) {
            this.$confirm(`确定删除 '${roleName}' 吗?`, '提示', {
                ...
                const result = await roleRequest.removeById(id)
        ...
        */
        removeRoles() {
            this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
                ...
                const result = await roleRequest.removeRoles(ids)
                this.getRoles()
                this.$message({
                    type: 'success',
                    message: '批量删除成功!'
