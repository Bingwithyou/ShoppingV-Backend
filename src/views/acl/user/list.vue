                ...
                <template slot-scope="{row}">
                    ...
                    <el-popconfirm :title="`确定删除 ${row.username} 吗?`" @onConfirm="removeUser(row.id)">
                        <el-button style="margin-left:10px" slot="reference" type="danger" size="mini"
                            icon="el-icon-delete" title="删除用户"></el-button>
                    </el-popconfirm>
                </template>
            </el-table-column>
        </el-table>
...
import userRequest from '@/api/acl/user'
...
export default {
    ...
    methods: {
        ...
        */
        async getRoles() {
            const result = await userRequest.getRoles(this.user.id)
        ...
        */
        async assignRole() {
            ...
            this.loading = true
            const result = await userRequest.assignRoles(userId, roleIds)
            this.loading = false
        ...
        */
        revomveUsers() {
            this.$confirm('确定删除吗?').then(async () => {
                await userRequest.removeUsers(this.selectedIds)
        ...
        */
        async removeUser(id) {
            await userRequest.removeById(id)
            this.$message.success('删除成功')
        ...
        */
        async getUsers(page = 1) {
            ...
            const result = await userRequest.getPageList(page, limit, searchObj)
        ...
        addOrUpdate() {
            this.$refs.userForm.validate(valid => {
                if (valid) {
                    const { user } = this
                    this.loading = true
                    userRequest[user.id ? 'update' : 'add'](user).then((result) => {
                        this.loading = false
